...
*   修订记录：
	2019-03-05：   1.0.0      build;
							  想使用tf.contrib.layers.batch_norm总是报错,无法UPDATE_OPS;
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        random_actions = tf.random_uniform(shape=tf.stack([batch_size]), minval=0, maxval=num_actions, dtype=tf.int64)
        chose_random = tf.random_uniform(shape=tf.stack([batch_size]), minval=0, maxval=1, dtype=tf.float32) < eps
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # optimize_expr = tf.group([optimize_expr, update_ops])
